document.addEventListener('DOMContentLoaded', function () {
    // Role checking is now handled by PHP session
    ...
    if (currentPath === 'index.php' || currentPath === 'data-mahasiswa.php' || currentPath === 'rekapitulasi.php' || currentPath === 'keuangan.php') {
        if (!checkUserRole(['admin'])) {
            return; // Hentikan eksekusi script jika tidak diizinkan
        }
    } else if (currentPath === 'dashboard-mahasiswa.php' || currentPath === 'profile-settings-mahasiswa.php' || currentPath === 'pembayaran.php') {
    ...
    const logoutLinks = document.querySelectorAll('[id^="mahasiswaLogout"], [id^="adminLogout"], #profileDropdown a[href="login.php"], #profileDropdownLogout');
    logoutLinks.forEach(link => {
        link.addEventListener('click', function (event) {
            ...
            window.location.href = 'login.php'; // Arahkan ke halaman login utama
        });
    });


    // --- Login Admin Form Logic (only runs on login.php) ---
    ...
    // --- Profile Settings Page Logic (for Admin, only runs on profile-settings.php) ---
    ...
    // --- Change Password Form Logic (for Admin, only runs on profile-settings.php) ---
    ...
    // --- Data Mahasiswa Page Logic (only runs on data-mahasiswa.php for Admin) ---
    ...
    // Panggil fungsi ini saat halaman dimuat untuk pertama kali jika di halaman data-mahasiswa.php
    ...
    // --- Login Mahasiswa Form Logic (only runs on login-mahasiswa.php) ---
    ...
    if (loginMahasiswaForm) {
        ...
        loginMahasiswaForm.addEventListener('submit', function (event) {
            ...
            if (foundMahasiswa) {
                ...
                setTimeout(() => {
                    window.location.href = 'dashboard-mahasiswa.php'; // Redirect ke dashboard mahasiswa
                }, 1500);
            } else {
    ...
    if (currentPath === 'dashboard-mahasiswa.php' && userRole === 'mahasiswa') {
        ...
        if (currentMahasiswa) {
            ...
            localStorage.removeItem('userRole');
            window.location.href = 'login-mahasiswa.php';
        }
    }


    // --- Profile Settings Mahasiswa Page Logic (only runs on profile-settings-mahasiswa.php) ---
    ...
    // --- Change Password Form Logic Mahasiswa (only runs on profile-settings-mahasiswa.php) ---
    ...
    // --- Informasi Pembayaran Page Logic (only runs on pembayaran.php for Mahasiswa) ---
    if (currentPath === 'pembayaran.php' && userRole === 'mahasiswa') {
        const loggedInMahasiswaNim = localStorage.getItem('loggedInUserNim');
        const loggedInMahasiswaName = localStorage.getItem('loggedInUser');
    ...
    if (userRole === 'mahasiswa') {
        ...
        if (profileSettingsLinkDropdown) {
            profileSettingsLinkDropdown.href = 'profile-settings-mahasiswa.php';
        ...
        const profileSettingsLinkDropdown = document.querySelector('#profileDropdown a[href="profile-settings-mahasiswa.php"]');
        if (profileSettingsLinkDropdown) {
            profileSettingsLinkDropdown.href = 'profile-settings.php';
            profileSettingsLinkDropdown.textContent = 'Pengaturan Profil'; // Pastikan teksnya benar
        }
